...
# Now you can use the LSTM class in your script
...
def generate_data(n_steps):
    time = torch.arange(0, n_steps)
    sine_wave = torch.sin(0.1 * time) + 0.2 * torch.randn(n_steps)
    return sine_wave.view(-1, 1, 1)
...
for epoch in range(epochs):
    ...
    # Generate target data (shifted by one time step)
    target = torch.cat([data[1:].view(-1, 1), torch.zeros(1, 1)], dim=0)
...
with torch.no_grad():
    test_data = generate_data(150)
    predicted_output, _ = model(test_data)

# Plot the results
plt.figure(figsize=(12, 6))
plt.plot(data[:, 0, 0], label='Original Data', linestyle='dashed')
plt.plot(range(n_steps, n_steps + len(predicted_output)), predicted_output[:, 0, 0], label='Predicted Data')
